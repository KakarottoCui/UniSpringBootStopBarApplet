...
<script>
	...
	export default {
		...
		computed: {
			...
			itemStyle() {
				return index => {
					...
					if (this.mode === 'box') {
						...
						style.borderLeft = `${this.hairline ? 0.5 : 1}px solid ${this.borderColor}`
						// 如果盒子间距为0的话
			...
			dooStyle() {
				const style = {}
				let newsize = this.size.substring(0,4)
				style.left = newsize * 2 + 'rpx'
				return style
			}
		},
		methods: {
